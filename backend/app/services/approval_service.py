...
class ApprovalService:
    ...
    async def save_pending(query_id: str, context: Dict[str, Any]) -> None:
        ...
        risk_assessment = ApprovalService.assess_sql_risk(sql_query, context.get("user_role", "analyst"))
        ...
        await redis_client.set(f"{ApprovalService.PREFIX}{query_id}", payload, ttl=ApprovalService.TTL)
        logger.info(f"Saved pending approval for query {query_id} with risk level: {risk_assessment.get('risk_level')}")
    ...
    @staticmethod
    async def mark_approved(query_id: str, sql_query: str, approver: str) -> bool:
        """
        ...
        await redis_client.set(key, approval_record, ttl=ApprovalService.IDEMPOTENCY_TTL)
        ...
        if pending:
            ...
            await redis_client.set(f"{ApprovalService.PREFIX}{query_id}", pending, ttl=ApprovalService.TTL)
        
        logger.info(f"Approval marked for query {query_id} by {approver}")
        return True
    ...
    async def reassess_modified_sql(
        ...
        # Compare risk levels
        risk_changed = new_assessment["risk_level"] != original_assessment.get("risk_level")
        risk_increased = ApprovalService._compare_risk_levels(
