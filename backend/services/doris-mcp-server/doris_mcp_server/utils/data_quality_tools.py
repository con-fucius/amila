        ...
        try:
            ...
            async with self.connection_manager.get_connection_context("query") as connection:
                ...
                if len(target_columns_info) > max_columns:
                    logger.warning(f" Column count ({len(target_columns_info)}) exceeds batch limit ({max_columns}), processing first {max_columns} columns")
    ...
    async def analyze_table_storage(
        ...
        try:
            ...
            async with self.connection_manager.get_connection_context("query") as connection:
                ...
                logger.info(" Analyzing physical distribution...")
            ...
            try:
                ...
                if result.data:
                    ...
                    if null_rate > 0.1:  # More than 10% null rate
                        logger.info(f"      High null rate: {null_rate:.1%}")
                    else:
                ...
                if result.data and result.data[0]["non_null_count"] > 0:
                    ...
                    if stats["min_date"] and stats["max_date"]:
                        try:
                            ...
                            logger.debug(f"Could not calculate time span for {col_name}: {e}")
